...
await updateDoc(washingtonRef, {
  ...
  date:allBlogsOfThisUser[blogIndex].date,
  firstName:allBlogsOfThisUser[blogIndex].firstName,
  lastName:allBlogsOfThisUser[blogIndex].lastName,
  userImage:allBlogsOfThisUser[blogIndex].userImage,
  userUid:allBlogsOfThisUser[blogIndex].userUid,
  id:allBlogsOfThisUser[blogIndex].id
});
console.log(allBlogsOfThisUser[blogIndex])
...
<Toaster position="top-right" reverseOrder={false} />
  ...
  <div className="modal-box max-w-lg mx-auto p-4 sm:p-6">
    ...
    >
      <div className="form-control mb-4">
        ...
        />
        {editBlogDefualtTitle !== '' ? (
          <span>Old Title: <b>{editBlogDefualtTitle}</b></span>
        ) : null}
        {edittingErrors.editTitle && (
      ...
      <div className="form-control mb-4">
        ...
        ></textarea>
        {editBlogDefualtBlog !== '' ? (
          <span>Old Description: <b>{editBlogDefualtBlog}</b></span>
        ) : null}
        {edittingErrors.editDescription && (
          <span className="text-red-500">This field is required.</span>
        )}
      </div>
      
      {editButtonLoadingCondition ? (
        <button
          type="submit"
          className="bg-indigo-600 w-full animate-pulse rounded-md text-white text-xl p-2"
          disabled
        >
          Editing...
        </button>
        ...
        <button
          type="submit"
          className="bg-indigo-600 w-full rounded-md text-white text-xl p-2"
        >
          Edit
        </button>
      )}
...
<dialog id="my_modal_4" className="modal">
  <div className="modal-box relative max-w-lg mx-auto p-6 bg-white rounded-lg shadow-lg">
    {/* Close button */}
    ...
    {isModalOpen ?  <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      <div className="flex flex-wrap justify-center gap-5 mt-5 flex-col">
        <div>
        <form
  className="flex flex-col space-y-4 p-4 sm:p-6 bg-white shadow-lg rounded-md max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-3xl mx-auto mt-5"
  onSubmit={handleSubmit(postBlog)}
  id="form"
>
  {/* Professional Heading with Icon */}
  <div className="flex flex-col sm:flex-row items-center justify-center sm:space-x-3 mb-6 text-center">
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="w-6 h-6 sm:w-8 sm:h-8 text-indigo-600 mb-2 sm:mb-0"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
    >
    ...
    <h1 className="text-lg sm:text-xl md:text-2xl font-semibold text-indigo-700">
      Post Your Blog Here
    </h1>
  </div>
  ...
  <div className="relative">
    <input
      ...
      className="text-md sm:text-lg md:text-xl pl-12 pr-4 py-2 w-full border-2 border-indigo-400 rounded-md focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 outline-none"
      {...register("title", { required: true })}
    />
    {errors.title && <span className="text-red-500">Title is required.</span>}
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="absolute left-3 top-3 w-6 h-6 sm:w-7 sm:h-7 text-gray-400"
      fill="none"
      viewBox="0 0 24 24"
      stroke="currentColor"
    >
  ...
  <div className="relative">
    ...
    ></textarea>
    {errors.blog && <span className="text-red-500">Blog is required.</span>}
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="absolute left-3 top-3 w-6 h-6 sm:w-7 sm:h-7 text-gray-400"
      fill="none"
  ...
  <h1 className="flex items-center space-x-2 text-xl sm:text-2xl md:text-3xl font-semibold mb-6 justify-center max-w-full sm:max-w-xl md:max-w-2xl mx-auto">
    <svg
      xmlns="http://www.w3.org/2000/svg"
  ...
  allBlogsOfThisUser.map((item, index) => {
    return (
      <div
        className="border border-gainsboro rounded-md shadow-sm p-6 bg-white max-w-md sm:max-w-md md:max-w-2xl mt-5 mb-5 mx-auto"
        key={index}
      >
        <div className="flex justify-end items-center">
          <div className="relative inline-block">
            ...
            {index === activeCardIndex && (
              <ul className="absolute right-0 mt-2 w-48 bg-white shadow-lg border border-gray-200 rounded-md">
                <li
                  className="flex items-center px-4 py-2 hover:bg-gray-200 cursor-pointer"
                  onClick={() => showEditModal(index, item.id)}
                >
                  <svg
